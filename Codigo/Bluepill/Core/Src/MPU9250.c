...
uint8_t MPU_begin(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250) {
	...
	if (check == WHO_AM_I_9250_ANS) {
		...
		MPU_writeAccFullScaleRange(SPIx, pMPU9250,
				pMPU9250->settings.aFullScaleRange);
		MPU_writeGyroFullScaleRange(SPIx, pMPU9250,
				pMPU9250->settings.gFullScaleRange);
		return 1;
	} else {
		return 0;
	}
...
void MPU_REG_WRITE(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250, uint8_t *pAddr,
		uint8_t *pVal) {
...
void MPU_REG_READ(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250, uint8_t addr,
...
void MPU_CS(MPU9250_t *pMPU9250, uint8_t state) {
	HAL_GPIO_WritePin(pMPU9250->settings.CS_PORT, pMPU9250->settings.CS_PIN,
			state);
}
...
void MPU_writeAccFullScaleRange(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250,
		uint8_t aScale) {
	// Variable init
...
void MPU_writeGyroFullScaleRange(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250,
		uint8_t gScale) {
	// Variable init
...
void MPU_calibrateGyro(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250,
		uint16_t numCalPoints) {
	// Init
	int32_t x = 0;
	int32_t y = 0;
	int32_t z = 0;
...
void MPU_readProcessedData(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250) {
	...
	pMPU9250->sensorData.ax = pMPU9250->rawData.ax
			/ pMPU9250->sensorData.aScaleFactor;
	pMPU9250->sensorData.ay = pMPU9250->rawData.ay
			/ pMPU9250->sensorData.aScaleFactor;
	pMPU9250->sensorData.az = pMPU9250->rawData.az
			/ pMPU9250->sensorData.aScaleFactor;
...
void MPU_calcAttitude(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250) {
	...
	pMPU9250->attitude.r = pMPU9250->attitude.tau
			* (pMPU9250->attitude.r
					- pMPU9250->sensorData.gy * pMPU9250->attitude.dt)
			+ (1 - pMPU9250->attitude.tau) * accelRoll;
	pMPU9250->attitude.p = pMPU9250->attitude.tau
			* (pMPU9250->attitude.p
					- pMPU9250->sensorData.gx * pMPU9250->attitude.dt)
			+ (1 - pMPU9250->attitude.tau) * accelPitch;
